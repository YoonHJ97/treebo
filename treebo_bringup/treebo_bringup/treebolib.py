...
class TreeboBase:
    """
    Treebo 전용 모터·IMU·엔코더·전압 제어/조회 드라이버 (단일 파일 버전).
    ...
    def __init__(
        ...
        if self._ser.is_open:
            print(f"TreeboBase: serial opened on {port} @ {baudrate}")
        else:
    ...
    def _build_frame(self, opcode: OpCode) -> TreeboFrameBuilder:
        return TreeboFrameBuilder(
    ...
    def start_background_reader(self) -> None:
        ...
        print("TreeboBase: background reader started")
    ...
    def close(self) -> None:
        ...
        if self._ser and self._ser.is_open:
            self._ser.close()
            print("TreeboBase: serial closed")
...
if __name__ == "__main__":
    bot = TreeboBase(port="/dev/ttyUSB0", car_type=TreeboBase.CARTYPE_X3, debug=True)
